        ...
        <div class="card card-body mx-3 mx-md-4 mt-n6">
            <div class="row gx-4 mb-2">
                <div class="col-auto">
                    <div class="position-relative">
                        @if ($userInfo->image)
                            <img id="image_preview" src="{{ asset('../storage/profile/' . $userInfo->image) }}"
                                alt="profile_image" class="rounded-circle shadow-sm"
                                style="border: 1px solid #16A085; width: 80px; height: 80px; -o-object-fit: cover; object-fit: cover; -o-object-position: center; object-position: center;">
                        @else
                ...
                <div class="col-auto my-auto">
                    ...
                    <div>
                        <div class="custom-file">
                            <input type="file" class="custom-file-input hidden" style="width: 0px;" accept="image/*"
                                id="image" name="image">
                            <label class="custom-file-label" for="image"><i class="bi bi-camera h-4"></i> Ubah
                                foto</label>
                            <label class="custom-file-label ps-5" id="dropdownMenuButton1" data-bs-toggle="dropdown"
                            ...
                            <ul class="dropdown-menu" style="border: 1px solid" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item deletePhoto" id="{{ $userInfo->id }}"
                                        style="font-size: 12px; color: red;" href="#"><i class="bi bi-trash"></i>
                                        Hapus Foto Profil</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-5 col-md-6 my-sm-auto ms-sm-auto me-sm-0 mx-auto mt-3">
                    <div class="nav-wrapper position-relative end-0">
                        <ul class="nav nav-fill p-1">
                            <li class="nav-item">
                                <a class="btn {{ Request::is('pembeli') ? 'active text-white bg-primary shadow' : 'border' }}"
                                    onclick="pembeli_dashboard('{{ url('pembeli') }}')" href="#">
                                    <span class="ms-1 fw-bold">Biodata Diri</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="btn {{ Request::is('pembeli/alamat*') ? 'active text-white bg-primary shadow' : 'border' }}"
                                    onclick="pembeli_alamat('{{ url('pembeli/alamat') }}')" href="#">
            ...
            <div class="row">
                <div class="col-12 mt-4">
                    ...
                    <div class="row mx-4 mx-sm-0 WishlistItems" id="showWhislist">
                        {{-- @if ($wishlist->count())
                                    ...
                                    <div class="card card-blog border rounded bg-white shadow card-plain {{ $item->product->stoke == 0 ? 'bg-light opacity-90' : '' }}"
                                        ...
                                        <div
                                            ...
                                            @php
                                                ...
                                                $ratingSum = App\Models\Review::where('product_id', $item->product->id)->sum('stars_rated');
                                                if ($reviews->count() > 0) {
                                            ...
                                            @endphp
                                            @for ($i = 1; $i <= $rateNum; $i++)
                                                <span><i class="lni lni-star-filled" style="color: #f0d800;"></i></span>
                                            @endfor
                                            @for ($j = $rateNum + 1; $j <= 5; $j++)
                                                <span><i class="lni lni-star"></i>
                                                </span>
                                            @endfor
...
@section('script')
    ...
    <script>
        ...
        var displayProduct = '{{ $wishlist }}';
        var displayProductNull = 4;
        if (displayProduct > 0) {
            $('#showWhislist').html(createSkeleton(displayProduct));
        } else {
            $('#showWhislist').html(createSkeleton(displayProductNull));
        }
        ...
        setTimeout(function() {
            fetchAllWishlist(displayProduct);
        }, 2000);

        function fetchAllWishlist(limit) {
            $.ajax({
                url: '{{ route('fetchAllWishlist') }}',
                data: {
                    limit: limit,
                },
                method: 'get',
                success: function(response) {
                    $("#showWhislist").html(response);
                }
            });
        }
