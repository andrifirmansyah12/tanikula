...
<section class="item-details mt-md-4 section bg-white overflow-hidden">
    <div class="container">
        <div class="bg-white">
            ...
            @csrf
                <input type="hidden" name="id_product" id="id_product" value="{{ $cartItem->id }}">
                <input type="hidden" name="product_qty" id="product_qty" value="{{ $cartItem->stoke }}">
                <div class="row">
                    <div class="col-12 col-xl-8 mb-3 mb-xl-0 mb-5 mb-md-0">
                                    ...
                                    <div class="row align-items-center">
                                        <div>
                                            <p class="fw-bold mb-2"><i class="bi bi-shop"></i>
                                                {{ $cartItem->user->name }}</p>
                                        </div>
                                        <div class="col-12 col-md-3 col-lg-3">
                                            <div class="d-flex align-items-center justify-content-center bg-light"
                                                style="width: 160px; height: 150px;">
                                                @if ($cartItem->photo_product->count() > 0)
                                                    @foreach ($cartItem->photo_product as $photos)
                                                        @if ($photos->name)
                                        ...
                                        <div class="col-6 mt-2 mt-md-0">
                                            <h3 class="mb-2 fs-6 fw-bold"><a style="color:#16A085;"
                                                    href="{{ url('home/'.$cartItem->slug) }}">{{ $cartItem->name }}</a>
                                            </h3>
                                            {{-- <p class="fw-bold">Type Produk</p> --}}
                                            <p style="font-size: 13px;">{{ $cartItem->stoke }} Barang</p>
                            ...
                            if($result_cost) {
                                foreach($result_cost as $result) {
                                    if ($result['service'] == $data_service) {
                                        $subTotal = $cartItem->price * $cartItem->stoke;
                                        $total += $cartItem->price * $cartItem->stoke;
                                        $totalService = $total + $result['cost'][0]['value'];
                                        $totalQty += $cartItem->stoke;
                                        $valueService = $result['cost'][0]['value'];
                                        $totalWeightProduct += $cartItem->weight;
                                    }
                                }
                            } else {
                                $subTotal = $cartItem->price * $cartItem->stoke;
                        ...
                        <div class="my-2">
                            <div class="d-flex justify-content-between">
                                <div class="fw-bold">Subtotal ({{$cartItem->stoke}} Barang)</div>
        ...
        <div class="modal-content">
            {{-- <form action="#" id="checkCourier" method="GET">
                ...
                <div class="modal-body">
                    ...
                    @else --}}
                    <form action="{{ route('buy.now.pembeli') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @foreach ($address as $item)
                        ...
                        <input type="hidden" name="prod_id" id="prod_id" value="{{ $cartItem->id }}">
                        <input type="hidden" name="quantity" id="quantity" value="{{ $cartItem->stoke }}">
                        <div class="my-2">
